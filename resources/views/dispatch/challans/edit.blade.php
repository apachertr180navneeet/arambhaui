...
@section('title', 'Edit Dispatch Challan - GarmentERP')
...
@section('content')
<form action="{{ route('dispatch.challans.update', $challan->id) }}" method="POST" id="dispatch-form">
  @csrf
  @method('PUT')
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:14px;">
      <div>
        <div style="display:flex; align-items:center; gap:8px;">
          ...
          <span style="font-family:var(--font-mono, monospace); font-weight:800; font-size:1rem; background:#f0fdf4; color:#15803d; padding:3px 10px; border-radius:8px; border:1px solid #bbf7d0;">
            {{ $challan->challan_no }}
          </span>
        </div>
        <p style="margin:4px 0 0; font-size:0.85rem; color:var(--slate-500);">Modify logistics details, LR numbers, carrier tracking, or consignee details.</p>
      </div>
      ...
      <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(220px, 1fr)); gap:18px;">
        ...
        <div class="form-group" style="margin-bottom:0;">
          ...
          <select name="customer_name" id="dc_customer" class="form-control" required onchange="onCustomerSelect(this)">
            <option value="{{ $challan->customer_name }}" selected>{{ $challan->customer_name }}</option>
            @foreach($customers as $c)
              @if($c->name !== $challan->customer_name)
        ...
        <div class="form-group" style="margin-bottom:0;">
          ...
          <input type="text" name="destination_city" id="dc_city" class="form-control" value="{{ $challan->destination_city }}" placeholder="e.g. Mumbai / Tiruppur / Bengaluru">
        </div>
    ...
    <div class="card" style="background:#fff; border-radius:var(--radius-xl); border:1px solid var(--slate-200); box-shadow:var(--shadow-sm); padding:24px;">
      ...
      <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(220px, 1fr)); gap:18px;">
        ...
        <div class="form-group" style="margin-bottom:0;">
          ...
          <input type="text" name="transporter_name" class="form-control" required value="{{ $challan->transporter_name }}" placeholder="e.g. V-Trans Logistics Ltd. / SafeXpress">
        </div>

        <div class="form-group" style="margin-bottom:0;">
          <label class="form-label" style="font-weight:700; color:var(--slate-800);">LR (Lorry Receipt) #</label>
          <input type="text" name="lr_number" class="form-control" value="{{ $challan->lr_number }}" placeholder="e.g. LR-987654" style="font-family:var(--font-mono, monospace); font-weight:700;">
        </div>

        <div class="form-group" style="margin-bottom:0;">
          <label class="form-label" style="font-weight:700; color:var(--slate-800);">Vehicle / Truck Number</label>
          <input type="text" name="vehicle_number" class="form-control" value="{{ $challan->vehicle_number }}" placeholder="e.g. TN-38-AB-1234" style="font-family:var(--font-mono, monospace); text-transform:uppercase;">
        </div>
    ...
    <div class="card" style="background:#fff; border-radius:var(--radius-xl); border:1px solid var(--slate-200); box-shadow:var(--shadow-sm); padding:24px;">
      ...
      <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(220px, 1fr)); gap:18px;">
        ...
        <div class="form-group" style="margin-bottom:0;">
          ...
          <input type="number" min="0" name="total_cartons" id="dc_cartons" class="form-control" required value="{{ $challan->total_cartons }}" oninput="updateSummary()" style="font-weight:700; font-size:1.05rem;">
        </div>
        ...
        <div class="form-group" style="margin-bottom:0;">
          ...
          <input type="number" min="0" name="total_qty" id="dc_qty" class="form-control" required value="{{ $challan->total_qty }}" oninput="updateSummary()" style="font-weight:700; font-size:1.05rem; color:#059669;">
        </div>
